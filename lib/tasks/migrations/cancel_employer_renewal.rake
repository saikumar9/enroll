...
# This rake task used to Cancel conversion employer renewals.
# ex: RAILS_ENV=production bundle exec rake migrations:conversion_employer_renewal_cancellations['043576862 474730282 042104074 042876321 043280601 043158592 043101467 270611091 300249279 274282797']
  desc "Cancel conversion employer renewals"
  task :conversion_employer_renewal_cancellations, [:fein] => [:environment] do |task, args|
    count = 0
    prev_canceled = 0
    feins = args[:fein].split(' ').uniq
    ...
    feins.each do |fein|
      ...
      if employer_profile.present?
        ...
        if plan_year.blank?
          plan_year = employer_profile.plan_years.published.detect{|py| py.start_on.year == 2018}
        end
        ...
        puts "EmployerProfile with fein: #{fein} Not Found" unless Rails.env.test?
      end
    end
