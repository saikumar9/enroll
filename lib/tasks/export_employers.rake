  ...
  task :export => [:environment] do
    ...
    CSV.open(file_name, "w") do |csv|
      ...
      headers = %w(employer.legal_name employer.dba employer.fein employer.hbx_id employer.entity_kind employer.sic_code employer_profile.profile_source employer.status ga_fein ga_agency_name ga_start_on
                                ...
                                office_location.address.city office_location.address.state office_location.address.zip office_location.address.county mailing_location.address_1 mailing_location.address_2 mailing_location.city mailing_location.state mailing_location.zip 
                                ...
                                benefit_group.reference_plan.name benefit_group.effective_on_kind benefit_group.effective_on_offset group_size_count participation_rate Employee-Only(Calculated_Premium) Employee-Spouse(Calculated_Premium) Employee-Children(Calculated_Premium) Family(Calculated_Premium) Employee-Only(New_Composite_Premium) Employee-Children(New_Composite_Premium) Employee-Spouse(New_Composite_Premium) Family(New_Composite_Premium) rate_basis_type
      ...
      orgs.all.each do |org| 
        begin
          ...
          employer_attributes << office_location.address.county if !office_location.address.county.blank?
          ...
          employer.plan_years.each_with_index do |plan_year, i|
            ...
            plan_year.benefit_groups.each do |benefit_group|
              ...
              group_size_count = benefit_group.export_group_size_count 
              participation_rate = benefit_group.participation_rate
                ...
                begin
                  ...
                  row += [benefit_group.title, benefit_group.plan_option_kind,
                          ...
                          benefit_group.reference_plan.name, benefit_group.effective_on_kind, benefit_group.effective_on_offset, group_size_count, participation_rate]
                  row += benefit_group.export_ctc_calculated
                  row += benefit_group.export_ctc_final
                  row += [benefit_group.export_rate_basis_type]
                  #row += benefit_group.sorted_composite_tier_contributions.map{|ctc| ctc.estimated_tier_premium}
