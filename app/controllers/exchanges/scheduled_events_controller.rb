  ...
  def index
    ...
    @calendar_events = @scheduled_events.flat_map do |e|
      if params.key?("start_date")
        e.calendar_events(Date.strptime(params.fetch(:start_date, Time.zone.now), "%m/%d/%Y").to_date, e.offset_rule)
      else
        e.calendar_events((params.fetch(:start_date, Time.zone.now)).to_date, e.offset_rule)
      end
    end
  end
