...
describe 'ModelEvents::InitialEmployerDenialNotice', dbclean: :around_each  do
  let(:model_event)  { "initial_employer_denial" }
  let(:notice_event) { "initial_employer_denial" }
      ...
      it "should trigger model event" do
        model_instance.observer_peers.keys.each do |observer|
          expect(observer).to receive(:plan_year_update) do |model_event|
            ...
            expect(model_event).to have_attributes(:event_key => :initial_employer_denial, :klass_instance => model_instance, :options => {})
          end
        end
  ...
  describe "NoticeTrigger" do
    context "when initial employer denial" do
      ...
      let(:model_event) { ModelEvents::ModelEvent.new(:initial_employer_denial, model_instance, {}) }

      it "should trigger notice event" do
        expect(subject).to receive(:notify) do |event_name, payload|
          expect(event_name).to eq "acapi.info.events.employer.initial_employer_denial"
