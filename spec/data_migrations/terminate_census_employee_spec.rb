...
describe TerminateCensusEmployee, dbclean: :after_each do
  ...
  describe "census employee's employment_terminated_on with past date" do
    ...
    let(:census_employee) { FactoryBot.create(:census_employee_with_benefit_group, employer_profile_id: employer_profile.id, employment_terminated_on: TimeKeeper::date_of_record - 5.days, hired_on: "2014-11-11") }
  ...
  describe "census employee's employment_terminated_on with future date" do
    ...
    let(:employer_profile) { FactoryBot.create(:employer_profile_default) }
    let(:employer_profile_id) { employer_profile.id }
    let(:census_employee) { FactoryBot.create(:census_employee_with_benefit_group, employer_profile_id: employer_profile.id, hired_on: "2014-11-11") }
    let(:census_employee_params) {
      {"first_name" => "aqzz",
       "middle_name" => "",
       "last_name" => "White",
